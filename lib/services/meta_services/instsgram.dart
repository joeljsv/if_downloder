...
import '../../model/instagram/instagram_user.dart'; // import http package for API calls
...
class InstaService {
  ...
  Future<InstagramReel> downloadReels(String link) async {
    ...
    InstagramReel reel = InstagramReel(
        thumbnailsrc: shortcodeMedia['display_url'],
        videourl: videoUrl,
        videoviewcount: shortcodeMedia['video_view_count'],
        videoplaycount: shortcodeMedia['video_play_count'],
        caption: shortcodeMedia['edge_media_to_caption']['edges'][0]['node']
            ['text'],
        user: InstagramUser(
            fullname: shortcodeMedia['owner']['full_name'],
            username: shortcodeMedia['owner']['username'],
            profilepicurl: shortcodeMedia['owner']['profile_pic_url'],
            edgefollowedby: shortcodeMedia['owner']['edge_followed_by']
                ['count'],
            biography: shortcodeMedia['owner']['biography'],
            externalurl: shortcodeMedia['owner']['external_url']));
    // write data to file

    return reel; // return download link
  }

  //get profile details
  Future<void> getProfileData(String username) async {
    var res = await http.get(Uri.parse(Uri.encodeFull(
        "$url$username/?__a=1&__d=dis"))); // adding /?__a=1&__d=dis at the end will return json data
